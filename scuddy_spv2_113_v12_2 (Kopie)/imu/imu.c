...
#include "utils.h"
#include "mc_interface.h" // added von Jörn
...
static void imu_read_callback(float *accel, float *gyro, float *mag) {
...
#ifdef IMU_REVERSE	// macht probleme mit Mag, da mag nicht jedes mal gelesen wird und dann invertiert wird.
	accel[0] *= -1.0;
	accel[1] *= -1.0;
	gyro[0] *= -1.0;
	gyro[1] *= -1.0;
	...
	gyro_rad[2] = m_gyro[2] * M_PI / 180.0;
		...
		// jörn accel von delta V mit einbeziehen MOD Jörn 2021 06 25
		static float ax_dyn;
		static float speed_last = 0;
		float speed_now = mc_interface_get_speed(); // in m/s
		static float filter_speed = 0.0;
		UTILS_LP_FAST(filter_speed, speed_now, 0.1);
		ax_dyn = (filter_speed - speed_last)/(dt*9.81);
		speed_last = filter_speed;
		utils_truncate_number(&ax_dyn, -0.25, 0.25);

		m_accel[0] +=  0.8*ax_dyn; // mit etwas gewichtung 
		
	
	if(m_settings.mode==AHRS_MODE_MADGWICK){
		ahrs_update_madgwick_imu(gyro_rad, m_accel, dt, (ATTITUDE_INFO*)&m_att);
		//ahrs_update_madgwick(gyro_rad, m_accel, m_mag, dt, (ATTITUDE_INFO*)&m_att);
		
	}
	
	if(m_settings.mode==AHRS_MODE_MAHONY){
		//ahrs_update_mahony_imu(gyro_rad, m_accel, dt, (ATTITUDE_INFO*)&m_att);
		ahrs_update_mahony(gyro_rad, m_accel, m_mag, dt, (ATTITUDE_INFO*)&m_att);
		//Debugging
		//ahrs_update_madgwick(gyro_rad, m_accel, m_mag, dt, (ATTITUDE_INFO*)&m_att);
