...
static THD_FUNCTION(timeout_thread, arg) {
	...
	for(;;) {
		if (timeout_msec != 0 && chVTTimeElapsedSinceX(last_update_time) > MS2ST(timeout_msec)) {
			mc_interface_unlock();
			mc_interface_set_reku(0);
			mc_interface_set_gas(0);
			mc_interface_set_enviro( (1 << 8) | (1 << 1) ); // ausschalten und Bremslicht an 
			static uint16_t hupe_timeout_counter=0;
			if (hupe_timeout_counter==500 && TIMEOUT_BEEP){ // nach fünf Sekunden erst melden
				mc_interface_set_enviro( (1 << 8) | (1 << 6) | (1 << 1) ); // ausschalten, Bremslicht an, Hupe piep,
			}
			hupe_timeout_counter++;
			if(hupe_timeout_counter==2000){ //10ms durchlaufzeit. alle 20sek. =2000
				hupe_timeout_counter=0;
			}
