...
void commands_process_packet(unsigned char *data, unsigned int len,
	...
	case COMM_GET_VALUES_SELECTIVE: {
		...
		if (mask & ((uint32_t)1 << 14)) {
			buffer_append_uint32(send_buffer, mc_interface_get_tachometer_abs_value(false), &ind);
		}
		if (mask & ((uint32_t)1 << 15)) {
			send_buffer[ind++] = mc_interface_get_fault();
		}
		if (mask & ((uint32_t)1 << 16)) { // Motor pos
			#define DEBUG_THIS_ON_PID_POS_OUTPUT 0 // ist unten nochmal !!
			
			if(DEBUG_THIS_ON_PID_POS_OUTPUT){ // debugging für // Maximum current right now in MC interface bei override limits
				buffer_append_float32(send_buffer, mc_interface_get_lo_current_motor_max_now(), 1e6, &ind);
			} else {
		...
		*/
		if (mask & ((uint32_t)1 << 18)) {
			//buffer_append_int16(send_buffer, mc_interface_get_gas()*1, &ind);
			buffer_append_int16(send_buffer, mc_interface_get_gas()*1 - mc_interface_get_reku()*1, &ind);
			//buffer_append_int16(send_buffer, app_adc_get_voltage()*1000, &ind);
			//buffer_append_int16(send_buffer, mc_interface_get_reku()*1, &ind);
			//buffer_append_int16(send_buffer, app_adc_get_voltage2()*1000, &ind);			
			buffer_append_float16(send_buffer, mc_interface_get_min_cell_voltage_bms(), 1e3, &ind);
			buffer_append_float16(send_buffer, mc_interface_get_max_cell_voltage_bms(), 1e3, &ind);
		}
	...
	case COMM_ALIVE:
		SHUTDOWN_RESET();
		/* // eeprom debug jörn 03 2021
			 commands_printf("SHUTDOWN_RESET");
			for(int i=0; i<10; i++){
				mc_interface_set_tachometer_abs_value(1);

				save_tacho_abs_value(mc_interface_get_tachometer_abs_value(false));

				mc_interface_get_tachometer_abs_value(true); // nullsetzen
				
				custom_configuration customconf;
				conf_general_read_custom_configuration(&customconf);// daten wiederholen
				mc_interface_set_tachometer_abs_value(customconf.tacho_abs_save);
			}
		}	
		*/	
		timeout_reset();
	...
	case COMM_GET_VALUES_SETUP_SELECTIVE: {
		...
		if (mask & ((uint32_t)1 << 10)) {
			//buffer_append_float32(send_buffer, val.ah_charge_tot, 1e4, &ind);					/// mod jörn BMS Spannung 
			buffer_append_float32(send_buffer, mc_interface_get_voltage_bms(), 1e4, &ind);
		}
		if (mask & ((uint32_t)1 << 11)) {
		...
		if (mask & ((uint32_t)1 << 15)) {
			if(DEBUG_THIS_ON_PID_POS_OUTPUT){ // debugging für // Maximum current right now in MC interface bei ovverride limits
				buffer_append_float32(send_buffer, mc_interface_get_lo_current_motor_max_now(), 1e6, &ind);
			} else {
				buffer_append_float32(send_buffer, mc_interface_get_pid_pos_now(), 1e6, &ind);
			}
