...
static float calculateAccConfidence(float accMag, float *accMagP) {
	...
	accMag = *accMagP * 0.9f + accMag * 0.1f; // alte magnitude mit 1/10 Filter verändern
	*accMagP = accMag;						// neuen wert als alt speichen

	confidence = 1.0 - (m_acc_confidence_decay * sqrtf(fabsf(accMag - 1.0f))); // wenn decay 1 dann bei 2g abs ergebnis = 0
