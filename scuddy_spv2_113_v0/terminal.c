...
void terminal_process_string(char *str) {
	...
	if (strcmp(argv[0], "ping") == 0) {
	...
	} else if (strcmp(argv[0], "tachoadd") == 0){
		if (argc == 2) {
			float tacho = -1.0;
			sscanf(argv[1], "%f", &tacho);
			//const float tacho_scale = 1000*(mcconf.si_motor_poles / 2.0 * mcconf.si_gear_ratio) / (mcconf.si_wheel_diameter * M_PI);
			//mc_interface_set_tachometer_abs_value((uint32_t)(tacho*tacho_scale));
			mc_interface_set_odo_value((uint32_t)(tacho*1000));
		...
		if (fault_vec_write == 0) {
			...
			for (int i = 0;i < fault_vec_write;i++) {
				...
				commands_printf("Voltage Display  : %.2f", (double)fault_vec[i].voltage_display);
				commands_printf("Voltage BMS      : %.2f", (double)fault_vec[i].voltage_bms);
				commands_printf("Voltage GAS      : %.2f", (double)fault_vec[i].voltage_gas);
				commands_printf("Voltage RECU     : %.2f", (double)fault_vec[i].voltage_recu);
				commands_printf("BMS FET Status   : %d", fault_vec[i].bms_fet_status);
				...
				if (fault_vec[i].fault == FAULT_CODE_DRV) {
					commands_printf("DRV8350_FAULTS   : %s ", drv8350_faults_to_string(fault_vec[i].drv8301_faults));
				}
#endif
				commands_printf(" ");
			}
	...
	} else if (strcmp(argv[0], "measure_ind2") == 0) {
		if (argc == 4) {
			float duty = -1.0;
			float samp = -1.0;
			float freq = -1.0;
			sscanf(argv[1], "%f", &duty);
			sscanf(argv[2], "%f", &samp);
			sscanf(argv[3], "%f", &freq);

			if (duty > 0.0 && duty < 0.9 && samp >= 0.0 && freq >= 0.0) {
				mcconf.motor_type = MOTOR_TYPE_FOC;
				//mcconf.foc_f_sw = freq;
				...
				float a_volt;
				float time;
				float ind = mcpwm_foc_measure_inductance2(freq, duty, samp, &curr, &a_volt, &time);
				commands_printf("Inductance: %.2f microhenry (%.2f , %.2f A, %.2fV, %.2fus)\n", (double)ind, (double)duty*100, (double)curr, (double)a_volt, (double)time*1000000) ;
	...
	// The help command
		...
		commands_printf("measure_ind2 [duty] [samples] [freqency]");
		commands_printf("  Jörn spezial :-) calculate the motor inductance");
...
void terminal_add_fault_data(fault_data *data) {
	...
	if (fault_vec_write >= FAULT_VEC_LEN) {
		fault_vec_write = FAULT_VEC_LEN-1;
	}
}

int terminal_get_fault_vec_write(void){
	return fault_vec_write;
}
