...
void mpu9150_get_accel(float *accel) {
	accel[0] = (float)raw_accel_gyro_mag[0] * 8.0 / 32768.0;
	accel[1] = (float)raw_accel_gyro_mag[1] * 8.0 / 32768.0;
	accel[2] = (float)raw_accel_gyro_mag[2] * 8.0 / 32768.0;
}

void mpu9150_get_gyro(float *gyro) {
	gyro[0] = (float)raw_accel_gyro_mag[3] * 1000.0 / 32768.0;
	gyro[1] = (float)raw_accel_gyro_mag[4] * 1000.0 / 32768.0;
	gyro[2] = (float)raw_accel_gyro_mag[5] * 1000.0 / 32768.0;
}
...
void icm_bank(uint_8 bank){
	write_single_reg(s, ICM20948_BANK_SEL, bank << 4);
	...
	//PWR_MGMT_1 in Header geändert 	Erik 22.09
	...
	// ACCEL_config BANK2 ADDR0x14
	//noch nicht geändert
	tx_buf[0] = MPU9150_ACCEL_CONFIG;
	tx_buf[1] = MPU9150_ACCEL_FS_8 << MPU9150_ACONFIG_AFS_SEL_BIT;
	res = i2c_bb_tx_rx(&i2cs, mpu_addr, tx_buf, 2, rx_buf, 0);
	...
	// GYRO CONFIG 1 BANK2 ADDR 0x01
	// noch nichts geändert
	tx_buf[0] = MPU9150_GYRO_CONFIG;
	tx_buf[1] = MPU9150_GYRO_FS_1000 << MPU9150_GCONFIG_FS_SEL_BIT;
	res = i2c_bb_tx_rx(&i2cs, mpu_addr, tx_buf, 2, rx_buf, 0);
	...
	//vermutlich  GYRO COnfig 1 BANK2 ADDR 0x01
	//noch nicht geändert
	...
	// is_mpu9250 = read_single_reg(MPU9150_WHO_AM_I) == 0x71; geändert erik 22.09
		
	is_mpu9250 = read_single_reg(MPU9150_WHO_AM_I) == 0xEA;
...
#if USE_MAGNETOMETER
static int get_raw_mag(int16_t* mag) {
	// gibt keinenn hxl??? hxl-->0x11? 0x0C das magnetometer?
