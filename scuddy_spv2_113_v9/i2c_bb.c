...
void i2c_bb_init(i2c_bb_state *s) {
	...
	//palSetPadMode(s->scl_gpio, s->scl_pin, PAL_MODE_OUTPUT_PUSHPULL);
	s->has_started = false;
	s->has_error = false;
}
...
bool i2c_bb_tx_rx(i2c_bb_state *s, uint16_t addr, uint8_t *txbuf, size_t txbytes, uint8_t *rxbuf, size_t rxbytes) {
	...
	//nack = i2c_write_byte(s, true, false, addr << 1);
	

	for (unsigned int i = 0;i < txbytes;i++) {
		i2c_write_byte(s, false, false, txbuf[i]);
		//nack += i2c_write_byte(s, false, false, txbuf[i]);
	}

	if (rxbytes > 0) {
		i2c_write_byte(s, true, false, addr << 1 | 1);
		//nack += i2c_write_byte(s, true, false, addr << 1 | 1);
	...
	return !nack; // true 1 = ok ### false 0 = fehler
}
...
static void i2c_delay(void) {
	//timer_sleep(1e-6); // 1µs
	timer_sleep(5e-7); // Jörn 2021 02 18
}
