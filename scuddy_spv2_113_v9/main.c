...
int main(void) {
	...
	chSysInit();
	 ...
	 spiInit(); // SPI initialisieren
	 uint8_t dataToSend[] = {0x01, 0x02, 0x03, 0x04}; // Beispiel-Daten
	     uint8_t receivedData[4]; // Empfangspuffer

	     // Daten senden
	     spiSendData(dataToSend, sizeof(dataToSend));

	     // Daten empfangen
	     spiReceiveData(receivedData, sizeof(receivedData));

	     // Hier könnte man die empfangenen Daten weiterverarbeiten...
