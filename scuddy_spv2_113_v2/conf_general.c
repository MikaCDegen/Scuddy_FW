...
void conf_general_init(void) {
	...
	for (unsigned int i = 0;i < (sizeof(custom_configuration) / 2);i++) {
 ...
 * true for success, false if variable was not found.
 */
void conf_general_read_custom_configuration(custom_configuration *conf) {
	bool is_ok = true;
	...
	if (!is_ok) {
		confgenerator_set_defaults_customconf(conf);
	}
}
 ...
 * true for success, false if something went wrong.
 */
bool conf_general_store_custom_configuration(custom_configuration *conf) {
	bool is_ok = true;
	uint8_t *conf_addr = (uint8_t*)conf;
	uint16_t var;
...
void save_odo_value(uint32_t value){
	...
	if(value > 100){ // debugging Jörn 2021 06 23 es konnte vorkommen, dass eine 0 gespeichert wurde.
		customconf.odo_save = value;
		if(!conf_general_store_custom_configuration(&customconf)){
