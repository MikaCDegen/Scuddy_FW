...
static float m_lambda_est;
static float m_l_calc;
...
static volatile uint32_t m_tachometer_abs;
...
void mcpwm_foc_set_current(float current) {
	if ( ( (mc_interface_get_enviro() & (1 << 14)) ) || ( (fabsf(current) < m_conf->cc_min_current) && ( (fabsf(mcpwm_foc_get_rpm())<100.0) || (mc_interface_get_fault() != FAULT_CODE_NONE) ) ) ) { // Mod von Jörn 08 2020
	//if ((fabsf(current) < m_conf->cc_min_current) && (fabsf(mcpwm_foc_get_rpm())<100.0) ) { // Mod von Jörn 07 2020: endstufe bleibt an, beim drehen
		m_control_mode = CONTROL_MODE_NONE;
		m_state = MC_STATE_OFF;
		stop_pwm_hw();
		return;
...
void mcpwm_foc_set_brake_current(float current) {
	if ( ( (mc_interface_get_enviro() & (1 << 14)) ) || ( (fabsf(current) < m_conf->cc_min_current) || (mc_interface_get_fault() != FAULT_CODE_NONE ) ) ){
		m_control_mode = CONTROL_MODE_NONE;
		m_state = MC_STATE_OFF;
		stop_pwm_hw();
		return;
...
float mcpwm_foc_get_mod_q_now(void) {
	return m_motor_state.mod_q;
}
...
void mcpwm_foc_set_tachometer_abs_value(int addition){
	m_tachometer_abs += addition;
}
...
float mcpwm_foc_measure_inductance(float duty, int samples, float *curr) {
	...
	float t = 2* ((float)TIM1->ARR * m_samples.measure_inductance_duty / (float)SYSTEM_CORE_CLOCK -
...
float mcpwm_foc_measure_inductance2(float freq, float duty, int samples, float *curr, float *a_volt, float *time) {
	m_samples.avg_current_tot = 0.0;
	m_samples.avg_voltage_tot = 0.0;
	m_samples.sample_num = 0;
	m_samples.measure_inductance_duty = duty;
...
void mcpwm_foc_print_state(void) {
	...
	commands_printf("Lambda_est:   %.2f", (double)(m_lambda_est*1e3));
	commands_printf("L_calc:       %.2f", (double)(m_l_calc*1e6));
}
...
//summe für dc cal bilden
...
#ifdef HW_HAS_3_SHUNTS
	float ic = ADC_curr_norm_value[2] * FAC_CURRENT;
#endif
	...
	if (m_state == MC_STATE_RUNNING) {
		...
		*/
		if (m_control_mode == CONTROL_MODE_CURRENT_BRAKE &&
				( ( (fabsf(mcpwm_foc_get_rpm())<50.0)  && !was_full_brake ) ||
				  ( (fabsf(mcpwm_foc_get_rpm())<300.0) &&  was_full_brake )  ) //NOTE MODIFIZIERT von Jörn Jacobi
				) {
			control_duty = true;
			duty_set = 0.0;
			was_full_brake = true;
		...
		if (control_duty) {
			...
			// Braking
			//static float iq_set_tmp_filter = 0.0;
...
/*
		...
		// Mindestdrehzahl für vollen Strom auf halben Abs_max Strom
		float rpm_now = 0.0;
			rpm_now = mcpwm_foc_get_rpm();
		if(fabsf(rpm_now) <= 200.0){
	...
	*/
	...
	if (diff > 3) {
		diff -= 6;
		//m_tachometer_abs++; // 1 tick je ERPM vorwäres
	} else if (diff < -2) {
		diff += 6;
		//m_tachometer_abs++;  // 1 tick je ERPM rückwärts
	}

	m_tachometer += diff;
	m_tachometer_abs += abs(diff); // 6 ticks je ERPM
...
static THD_FUNCTION(timer_thread, arg) {
	...
	for(;;) {
		...
		if (fabsf(m_pll_speed) < min_rads && min_rpm_hyst_timer < m_conf->foc_sl_openloop_hyst ) {
...
void observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta,
	...
	float L = (3.0 / 2.0) * m_conf->foc_motor_l;
...
//  OBSERVER AB FIRMWARE 1.08 2023 07 10 JJ
#else
void observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta,
		float dt, volatile float *x1, volatile float *x2, volatile float *phase) {
	...
	#ifdef LD_LQ_DIFF
	float ld_lq_diff = LD_LQ_DIFF ;
	// Adjust inductance for saliency.
	if (fabsf(m_motor_state.id) > 0.1 || fabsf(m_motor_state.iq) > 0.1) {
		L = L - ld_lq_diff / 2.0 + ld_lq_diff * SQ(m_motor_state.iq) / (SQ(m_motor_state.id) + SQ(m_motor_state.iq)); // kann nicht stimme, aber so aus 6.05 übernommen
	}
	#endif

	m_l_calc = L; // global specieern zum Auswerten
	const float L_ia = L * i_alpha;
	const float L_ib = L * i_beta;
	const float R_ia = R * i_alpha;
	const float R_ib = R * i_beta;
	...
	m_lambda_est += 0.2 * gamma_half * m_lambda_est * -err * dt;
